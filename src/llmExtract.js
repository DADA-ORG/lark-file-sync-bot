  ...
  input_schema: {
    ...
    properties: {
      ...
      update_summary: {
        type: 'string',
        description:
          '如实转述群消息里和这条岗位更新有关的内容，只做必要的语言整理（比如去掉"@机器人"这种指令性文字、口语化的"呃/那个"之类填充词），不要删减信息、不要替换成不同的说法、不要自行归纳精简或补充没提到的内容。除去指令性文字外，尽量保留原文的用词和信息量。',
      },
      raw_company_guess: {
        type: 'string',
...
async function extractAndMatch(messageText, jobsList) {
  ...
  const systemPrompt = `你是一个帮猎头/招聘顾问团队做岗位更新归档的助手。你会收到一条飞书群消息原文，以及一份当前有效的岗位候选记录列表。你的任务是判断这条消息说的是候选列表里的哪一条记录（可能因为顾问打字口语化、简称，需要你根据语义判断，不要求逐字匹配），并如实转述更新内容——这是归档记录，不是摘要，不要擅自删减信息、不要替换措辞、不要过度润色，只去掉和岗位更新本身无关的指令性文字（比如@机器人）。`;
